...
class WordLadderState(AbstractState):
        ...
        for word_idx in range(len(self.state)):
            ...
            for c_idx in range(97, 97+26):
                ...
                if is_english_word(potential_nbr):
                    ...
                    nbr_states.append((new_state, edge_cost))
        return nbr_states
    ...
    def __lt__(self, other):    
        ...
        return (self.dist_from_start + self.h, self.tiebreak_idx) < (other.dist_from_start + other.h, other.tiebreak_idx)
...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        r, c = self.zero_loc
        if r + 1 < len(self.state):
            new_state = copy.deepcopy(self.state)
            new_state[r][c], new_state[r+1][c] = new_state[r+1][c], new_state[r][c]
            new_zero_loc = [r+1, c]
            nbr_states.append(EightPuzzleState(new_state, self.goal, self.dist_from_start + 1, self.use_heuristic, new_zero_loc))
        if c - 1 >= 0:
            new_state = copy.deepcopy(self.state)
    ...
    def compute_heuristic(self):
        ...
        for i in range(len(self.goal)):
            for j in range(len(self.goal[0])):
                goal_positions[self.goal[i][j]] = (i, j)
        for i in range(len(self.state)):
            for j in range(len(self.state[0])):
                tile = self.state[i][j]
                if tile != 0:
                    total += manhattan((i, j), goal_positions[tile])
        return total
