...
def get_polynomial_features(x, degree):
    return np.concatenate([x ** i for i in range(degree, -1, -1)], axis=1)
...
def ik_loss(arm : Arm, config, goal):
    return np.linalg.norm(arm.forward_kinematics(config)[-1] - goal)
...
def estimate_ik_gradient(loss, config, num_samples):
    ...
    return gradient / np.linalg.norm(samples[max_loss_idx] - config)
